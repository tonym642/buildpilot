...
import AuthPanel from "./components/AuthPanel"; // Importing AuthPanel
...
export default function BuildPilot() {
  ...
  useEffect(() => {
    // If running in fallback mode (no Supabase), auto-sign in with fake user
    ...
    if (isFallback && typeof window !== "undefined") {
      setSession({ user: { id: "local-fake", email: "local@dev" } });
